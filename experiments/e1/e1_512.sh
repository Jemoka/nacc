...
for lr in 5e-3 5e-4 5e-5 5e-6; do
    for featureset in neuralpsych combined; do
        CUDA_VISIBLE_DEVICES=0 uv run python main.py "e0_${HIDDEN}_${LR}" --hidden_dim $HIDDEN --n_layers $NLAYERS --batch_size $BATCH_SIZE --lr $lr --featureset $featureset --wandb 
    done
done
